			...
			{tutorials.map((item, index) => {
				...
				return (
					<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} key={index}>
						<Card
							className='h-full group bg-gradient-to-br from-card to-muted/50 border border-border rounded-[6px] shadow-sm hover:border-primary/30 hover:bg-accent/50 transition-all duration-200 cursor-pointer hover:shadow-lg flex flex-col max-w-[280px] mx-auto p-4'
