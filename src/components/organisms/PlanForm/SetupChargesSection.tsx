...
export const AddChargesButton = ({ onClick, label, className }: AddChargesButtonProps) => (
	<button
		onClick={onClick}
		className={cn(
			'shrink-0 cursor-pointer flex gap-2 items-center justify-center bg-muted rounded-[6px] px-2.5 h-9 w-fit text-start',
			className,
		)}>
						...
						{subscriptionTypeOptions.map((type) => (
							<button
								...
								className={cn(
									...
									subscriptionType === type.value ? 'border-foreground' : 'border-border',
								)}>
								{type.icon && <type.icon size={24} className='text-foreground' />}
								<p className='text-foreground font-medium mt-2'>{type.label}</p>
