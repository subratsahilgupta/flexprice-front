...
import { formatBillingPeriodForDisplay, getCurrencySymbol } from '@/utils/common/helper_functions';
	...
	const columns: ColumnData<LineItemWithStatus>[] = useMemo(
		() => [
				...
				: []),
			{
				title: 'Price Type',
				render: (row) => (
					<PriceTypeChipsCell priceType={row.price_type ?? row.price?.type} invoiceCadence={row.price?.invoice_cadence} t={t} />
				),
			},
			{
