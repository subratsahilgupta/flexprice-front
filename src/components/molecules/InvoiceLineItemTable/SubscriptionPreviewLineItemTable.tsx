				...
				<table className='w-full border-collapse'>
					<thead>
						<tr className='border-b border-border'>
							<th className='py-3 px-0 text-start text-sm font-medium text-foreground'>{t(`${li}.subscription`)}</th>
							{invoiceType === INVOICE_TYPE.SUBSCRIPTION && (
								<th className='py-3 px-4 text-start text-sm font-medium text-foreground'>{t(`${li}.description`)}</th>
							)}
							{invoiceType === INVOICE_TYPE.SUBSCRIPTION && (
								<th className='py-3 px-4 text-start text-sm font-medium text-foreground'>{t(`${li}.interval`)}</th>
							)}
							<th className='py-3 px-4 text-center text-sm font-medium text-foreground'>{t(`${li}.quantity`)}</th>
							<th className='py-3 px-0 text-end text-sm font-medium text-foreground'>{t(`${li}.amount`)}</th>
						</tr>
					</thead>
					<tbody>
						{displayData?.map((item, index) => {
							return (
								<tr key={index} className='border-b border-border'>
									<td className='py-4 px-0 text-sm text-foreground'>{item.display_name ?? t('common:labels.na')}</td>
									{invoiceType === INVOICE_TYPE.SUBSCRIPTION && (
									...
									<td className='py-4 px-0 text-end text-sm text-foreground '>{formatAmount(item.amount ?? 0, item.currency)}</td>
								</tr>
							);
						})}
					</tbody>
				...
				<div className='w-80 space-y-2'>
					...
					{subtotal !== undefined && subtotal !== null && Number(subtotal) !== 0 && (
						<div className='flex flex-row justify-end items-center py-1'>
							<div className='w-40 text-end text-base font-medium text-foreground'>{t(`${li}.subtotal`)}</div>
							<div className='flex-1 text-end text-sm text-foreground font-medium'>{formatAmount(Number(subtotal), currency ?? '')}</div>
						</div>
					)}
					...
					{discount && Number(discount) > 0 && (
						<div className='flex flex-row justify-end items-center py-1'>
							<div className='w-40 text-end text-base font-medium text-foreground'>{t(`${li}.discount`)}</div>
							<div className='flex-1 text-end text-sm text-foreground font-medium'>−{formatAmount(Number(discount), currency ?? '')}</div>
						</div>
					)}
					{/* Tax - only show if provided and > 0 */}
					{tax !== undefined && tax !== null && Number(tax) !== 0 && (
						<div className='flex flex-row justify-end items-center py-1'>
							<div className='w-40 text-end text-base font-medium text-foreground'>{t(`${li}.tax`)}</div>
							<div className='flex-1 text-end text-sm text-foreground font-medium'>{formatAmount(Number(tax), currency ?? '')}</div>
						</div>
					)}

					{/* Net payable - always show, default to 0 if not provided */}
					<div className='flex flex-row justify-end border-t border-border items-center py-3'>
						<div className='w-40 flex items-center gap-2 justify-end text-sm text-foreground font-medium'>{t(`${li}.netPayable`)}</div>
						<div className='flex-1 text-end text-sm text-foreground font-semibold'>
