	...
	return (
		<div>
						...
						<>
							{isServiceAccountsError ? (
								<div className='bg-warning-muted border border-warning-line rounded-md p-3'>
									<div className='flex items-start gap-2'>
										<AlertTriangle className='w-4 h-4 text-warning-bright flex-shrink-0 mt-0.5' />
									...
									{selectedServiceAccount && selectedServiceAccount.roles && selectedServiceAccount.roles.length > 0 && (
										<div className='space-y-2'>
											<label className='block text-sm font-medium text-content-secondary'>
												{t('developers:labels.accountRolesPermissions')}
											</label>
											<div className='bg-info-muted border border-info-line rounded-md p-3'>
												<div className='flex items-start gap-2'>
													<Info className='w-4 h-4 text-info flex-shrink-0 mt-0.5' />
			...
			<Modal isOpen={isModalOpen} onOpenChange={setIsModalOpen}>
				<div className='space-y-4 bg-surface card p-5 max-w-md mx-auto'>
					<h1 className='text-xl font-semibold mb-4'>{t('developers:secretKeyDrawer.modal.title')}</h1>

					<div className='bg-warning-muted border border-warning-line rounded-md p-3 flex items-start gap-2'>
						<AlertTriangle className='w-5 h-5 text-warning-bright flex-shrink-0 mt-0.5' />
						<p className='text-sm text-warning-deep'>{t('developers:secretKeyDrawer.modal.warning')}</p>
					</div>

					<div className='mt-4'>
						<label className='block text-sm font-medium mb-1'>{t('developers:labels.apiKey')}</label>
						<div className='relative bg-surface-shell rounded-md'>
							<Input
								value={showApiKey ? data?.api_key || '' : maskApiKey(data?.api_key || '')}
								readOnly
								className='pr-16 border-none text-content-tertiary'
							/>
							<div className='bg-surface-shell absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1'>
								<button onClick={toggleApiKeyVisibility} className='p-1 text-content-secondary hover:text-content-secondary' type='button'>
									{showApiKey ? <EyeOff className='w-4 h-4' /> : <Eye className='w-4 h-4' />}
								</button>
								<button onClick={copyApiKey} className='p-1 text-content-muted hover:text-content-secondary' type='button'>
