...
const SelectMeter: FC<Props> = ({ onChange, value, error, label = 'Features', placeholder = 'Select feature', description, className, disabledFeatures }) => {
    const {
        data: featuresData,
        isLoading,
        isError,
    ...
    const options = featuresData.items
        .map((feature: Feature): SelectOption => ({
            value: feature.id,
            label: feature.name,
