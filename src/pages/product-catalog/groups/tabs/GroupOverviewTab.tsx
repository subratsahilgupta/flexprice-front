					...
					if (!isNaN(d.getTime()))
						items.push(
							<div key='start' className='flex items-center gap-2'>
								<span className='text-xs font-medium text-muted-foreground'>{t('common:labels.start')}</span>
					...
					if (!isNaN(d.getTime()))
						items.push(
							<div key='end' className='flex items-center gap-2'>
								<span className='text-xs font-medium text-muted-foreground'>{t('common:labels.end')}</span>
								<span className='text-sm font-medium'>{formatDateTimeWithSecondsAndTimezone(d)}</span>
							</div>,
						);
				} catch {
